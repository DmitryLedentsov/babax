...
typedef struct
{
    ...
    PhysicsManifoldData manifolds[BABAX_MAX_MANIFOLDS]; // Physics manifolds pointers array
    unsigned int manifoldsCount;                        // Physics world current manifolds counter
...
BABAXDEF int GetPhysicsShapeVerticesCount(PhysicsState state, int index)
{
    ...
    if (index < state->physicsBodiesCount)
    {
        if (state->bodies[index] != NULL)
        {
            ...
            {
            case PHYSICS_CIRCLE:
                result = BABAX_CIRCLE_VERTICES;
                break;
            case PHYSICS_POLYGON:
                result = state->bodies[index]->shape.vertexData.vertexCount;
                break;
            default:
                break;
            }
...
BABAXDEF Vector2 GetPhysicsShapePosition(PhysicsBody body)
{
    Vector2 position = {0.0f, 0.0f};
    ...
    {
        PolygonData vertexData = body->shape.vertexData;
        position = Vector2Add(body->position, Mat2MultiplyVector2(body->shape.transform, (Vector2){0, 0}));
    }
#if defined(BABAX_DEBUG)
    else
        printf("[BABAX] error when trying to get a null reference physics body");
...
// TODO: GetPhysicsShapeVertexPosition() and GetPhysicsShapeVertexTransformedPosition()sfasf
...
inline static void ProcessManifold(PhysicsState state, PhysicsManifold manifold)
{
    ...
    if (manifold->contactsCount <= 0)
        return;
    state->manifolds[state->manifoldsCount] = *manifold;
    state->manifoldsCount++;
...
static void PhysicsStep(PhysicsState state)
{
    ...
    for (int i = 0; i < state->physicsBodiesCount; i++)
    {
        ...
        if (bodyA != NULL)
        {
            ...
            IntegratePhysicsForces(state, bodyA);
            // Integrate velocity to physics bodies
            IntegratePhysicsVelocity(state, bodyA);
            // Clear physics bodies forces

            bodyA->force = BABAX_VECTOR_ZERO;
            bodyA->torque = 0.0f;
        }
    }
}
...
BABAXDEF Vector2 Vec2Multiply(Vector2 v1, float c)
{
